...
Use `./run-dev.sh` for the interactive Docker dev workflow. It clears `logs/dev/`, starts the backend `dotnet watch` container plus the Vite HMR container, and attaches to a split-pane `tmux` session that tails the dev launcher log, container logs, and app file log. Set `PYRITE_NO_TMUX=1` only for non-interactive automation.
